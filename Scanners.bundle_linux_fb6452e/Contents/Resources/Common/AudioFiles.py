    ...
    try:
      ...
      if track == None:
        ...
        #HACK
          ...
          # capturing group that matches any character that is not a digit between 0 and 9, zero or more times.
...
def getInfoFromTag(filename, language):
  ...
  #HACK
    ...
    try:
      ...
      return (None, None, None, None, None, None, None)
    artist = getWMAstring(mutagenGrabber(tag, 'ARTIST', language))
    album = getWMAstring(mutagenGrabber(tag, 'ALBUM', language))
    title = getWMAstring(mutagenGrabber(tag, 'TITLE', language))
    track = cleanTrackAndDisk(mutagenGrabber(tag, 'TRACK', language))
    disc = cleanTrackAndDisk(mutagenGrabber(tag, 'DISCNUMBER', language))
    TPE2 = getWMAstring(mutagenGrabber(tag, 'AlbumArtist', language))
    return (artist, album, title, track, disc, TPE2, compil)
